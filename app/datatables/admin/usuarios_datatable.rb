class Admin::UsuariosDatatable
    ...
    private 
            ...
            usuarios.each_with_index.map do |usuario, index|
                {
                    ...
                    'secretaria' => column_secretaria(usuario),
                ...
                def column_permissao(usuario)
                    ...
                    enum_i18n(Usuario, :permissoes, usuario.try(:permissao))
                    
                end


                def column_secretaria(usuario)
                    
                    usuario.try(:secretaria).try(:nome)
        ...
        def column_opcoes(usuario)
            ...
            opcoes << (link_to(admin_usuario_path(usuario),
            ...
            end).to_s if can? :show, usuario

            opcoes << (link_to(edit_admin_usuario_path(usuario),
                { remote: @remote, class: 'btn btn-sm btn-warning text-dark me-2', title: 'Editar',
                data: { toggle: 'tooltip', placement: 'top' } }) do
                content_tag(:i, '', class: 'bi bi-pencil') + ' Editar'
            end).to_s if can? :edit, usuario

            opcoes << (button_to admin_usuario_path(usuario),
                method: :delete,
            ...
            end).to_s if can? :destroy, usuario
        ...
        def sort_column
            columns = ["created_at", "nome", "permissao", "secretaria", "email"]
